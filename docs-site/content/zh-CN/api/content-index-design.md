...
外部内容源如果需要随正文发布图片或其他静态文件，可以额外实现 `getAssets(context)`，返回
`{ filePath, publicPath }` 描述。Core 会在开发预览和生产构建前复制资源；适配器负责把 Markdown 中的
相对引用改写为公开路径，资源发布不应把外部 checkout 目录直接挂到站点根目录。
