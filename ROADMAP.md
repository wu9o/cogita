...
本文顶部的“下一阶段”是当前可执行路线图。下方的完整架构树和历史阶段记录保留为
背景资料，其中列出的候选主题、插件和工具不代表当前仓库已经实现，也不作为发布包清单。
当前可安装能力以 [包与能力地图](./docs-site/content/package-map.md) 和各包的 `package.json`
为准。

## 下一阶段：产品化与生态采用（2026-08 起）

架构基础阶段已经完成。下一阶段的目标不是继续堆叠孤立插件，而是让 Cogita 成为可被真实站点长期消费、可被第三方扩展、升级过程可控的内容站点平台。
...
- [x] 使用 tarball 验证最小博客、独立博客和独立文档消费者。
- [x] 将独立博客消费者接入发布 CI，缺少真实内容仓库时阻断检查。
- [x] 增加首页、文章、搜索、标签、分类、归档和子路径部署的浏览器级验收。

### P1：开发者体验与采用入口

- [x] 提供博客站和文档站的 CLI 初始化模板。
...
- [ ] 将文章、文档页和知识条目统一为可被主题消费的内容索引模型。
- [x] 提供 `@cogita/plugin-content-relations`，生成站内出链、相关文章和反向链接数据。
- [ ] 开发 `@cogita/theme-knowledge`，把搜索、标签和内容关系组合成知识库信息架构。
- [ ] 用独立知识库消费者验证根路径、子路径、断链诊断和长期升级流程。

### P2：规模化性能基线

- [ ] 使用 100、1,000、10,000 篇文章建立构建和增量重建基准。
- [ ] 根据基准结果决定正文索引、虚拟模块和图片优化策略。

SSR、WASM、企业多站点和 AI 内容能力暂不作为当前阶段目标，待公共契约和真实站点采用数据稳定后再评估。

## 🏗️ 历史架构规划与候选能力

> 本节是历史规划记录，不代表当前实现状态。新增主题或插件前，应先更新顶部路线图，
> 并通过独立消费者和浏览器验收；不要仅根据本节的候选名称判断包是否存在。
...
- [x] `@cogita/plugin-blog-list` ✅ 已实现一期列表、分页和时间归档
  - [x] 分页的文章列表
  - [x] 稳定排序和列表配置
  - [x] 自定义布局和模板
  - [x] 按年归档页面
...
- [x] `@cogita/plugin-tags` ✅
  - [x] 自动标签提取
  - [x] 标签页面生成（React 组件布局）
  - [x] 交互式标签云组件（TagCloud / TagList）
  - [x] 基于标签的相关文章
...
- **📦 包数量**：7 个核心包已发布
- **📄 文档**：15万+ 字技术文档
- **🔌 插件**：3 个核心插件（posts-frontmatter、rss、tags），3 个开发中
